...
export class ApiService {
  ...
  // ── Market ────────────────────────────────────────────────────────────────
  getCandles(symbol: string, interval = '1m', from?: string, to?: string, limit = 500): Observable<Candle[]> {
    let params = new HttpParams().set('interval', interval).set('limit', limit);
    if (from) params = params.set('from', from);
    if (to) params = params.set('to', to);
    return this.http.get<Candle[]>(`${this.base}/market/candles/${symbol}`, { params });
  }
  ...
  startStrategy(name: string, symbols: string[]): Observable<void> {
    return this.http.post<void>(`${this.base}/strategies/${name}/start`, { symbols });
  }
