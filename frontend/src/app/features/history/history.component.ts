...
@Component({
        ...
        <table class="data-table">
          ...
          <tbody>
            @for (o of filtered(); track o.id) {
              ...
              <td>{{ o.filled_avg_price ? (o.filled_avg_price | number:'1.2-2') : '\u2014' }}</td>
                <td>
  ...
  styles: [`
    ...
    .filter-input, .filter-select { padding: 0.4rem 0.6rem; border-radius: 4px; border: 1px solid #45475a; background: #181825; color: #cdd6f4; }
    .state-msg { color: #6c7086; text-align: center; padding: 2rem; }
...
export class HistoryComponent implements OnInit {
  ...
  readonly filtered = computed(() =>
    this.orders().filter(o => {
      const symMatch = !this.filterSymbol || o.symbol.includes(this.filterSymbol.toUpperCase());
